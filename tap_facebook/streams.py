...
class AdsInsightStream(FacebookStream):
    ...
    def get_records(self, context: dict | None) -> t.Iterable[dict[str, t.Any]]:
        ...
        existing_context = self.get_context_state(context)
        last_sync_date = None

        # If context exists, get the last_sync_time from the state
        if existing_context:
            try:
                last_sync_date = parse_date(
                    existing_context["context"]["last_sync_time"]
                )
            except KeyError:
                pass

        # set the last sync date if it is available
        if last_sync_date:
            logger.info(f"Last sync date is: {last_sync_date}")
        ...
        # New logic to handle start_date and time ranges
        if not start_date:
        ...
        # Ensure 'since' is not after 'until'. If it is, swap them.
        if since > until:
    ...
    def get_url(self, context: dict | None) -> str:
        ...
        url = url + f"&time_range={context['time_range']}"
        return url
    ...
    def get_url_params(
        ...
        # params["time_range"] = context["timerange"]
